...
describe("auth functions", () => {
  ...
  describe("searchIdentities", () => {
    it("should search identities with correct parameters and return expected result", async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith("https://dev.azure.com/test-org/_apis/identities?api-version=7.1-preview.1&searchFilter=General&filterValue=john.doe%40example.com", {
        headers: {
          "Authorization": "Bearer fake-token",
          "Content-Type": "application/json",
          "User-Agent": "Jest",
    ...
    it("should properly encode search filter in URL", async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        "https://dev.azure.com/test-org/_apis/identities?api-version=7.1-preview.1&searchFilter=General&filterValue=user+with+spaces%40example.com",
        expect.any(Object)
      );
    });
  });
  ...
  describe("getUserIdFromEmail", () => {
    ...
    it("should work with unique names as well as emails", async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith("https://dev.azure.com/test-org/_apis/identities?api-version=7.1-preview.1&searchFilter=General&filterValue=john.doe", expect.any(Object));
