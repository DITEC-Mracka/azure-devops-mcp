...
import type { IHttpClient, IHttpClientResponse, IRequestHandler, IRequestInfo } from "typed-rest-client/Interfaces.js";
 ...
 */
export class SspiRequestHandler implements IRequestHandler {
  private serverUrl: string;
  private winSsoModule: typeof import("win-sso") | null = null;
   ...
   */
  private async loadWinSso(): Promise<typeof import("win-sso")> {
    if (this.winSsoModule) return this.winSsoModule;
    try {
  ...
  canHandleAuthentication(response: IHttpClientResponse): boolean {
    if (response.message.statusCode !== 401) return false;
  ...
  async handleAuthentication(httpClient: IHttpClient, requestInfo: IRequestInfo, objs: unknown): Promise<IHttpClientResponse> {
    ...
    let peerCert: import("tls").PeerCertificate | undefined;
    ...
    let sso: InstanceType<typeof winSso.WinSso> | undefined;
    try {
      ...
      (requestInfo.options.headers as Record<string, string>)["Authorization"] = authRequestHeader;

      let response = await httpClient.requestRaw(requestInfo, objs as string);
      ...
      while (response.message.statusCode === 401 && rounds < maxRounds) {
        ...
        (requestInfo.options.headers as Record<string, string>)["Authorization"] = responseHeader;
        response = await httpClient.requestRaw(requestInfo, objs as string);
        rounds++;
      }
