...
let isOnPrem = false;
try {
    const resolved = resolveOrgUrl(orgName, argv.allowHttp);
...
async function main() {
    ...
    if (effectiveAuthType === "pat") {
        ...
        function rawHttpRequest(url, method, headers, body, agent) {
    ...
    if (isOnPrem) {
        try {
            ...
            if (effectiveAuthType === "sspi") {
                ...
                const response = await fetch(checkUrl, { method: "GET", headers, signal: AbortSignal.timeout(10_000), dispatcher: onPremDispatcher });
                if (!response.ok) {
                    logger.error(`On-prem connection check failed: HTTP ${response.status} ${response.statusText} from ${checkUrl}`);
                    process.exit(1);
                }
